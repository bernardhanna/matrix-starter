...
test('open pickup Select2 control stays in flow instead of covering the menu', function () use ($express_css) {
    ...
    expect($express_css)->toContain('font-weight: 700 !important');
    expect($express_css)->toContain('-webkit-text-fill-color: #000 !important');
    expect($express_css)->toContain('content: none');
    expect($express_css)->toContain('#rd-checkout-step-method .pickup-location-field .select2-selection__placeholder');
});
