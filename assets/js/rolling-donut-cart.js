/**
 * Rolling Donut cart page enhancements.
 * Relies on WooCommerce cart.js (wc-cart) for AJAX updates.
 */
jQuery(function ($) {
  ...
  function requestCartUpdate() {
    ...
    // WooCommerce cart.js only processes quantity changes when update_cart is submitted.
    ...
    $('input.qty').not('.hasQtyButtons').each(function () {
      ...
      $('<button type="button" class="border border-solid border-black rounded bg-white h-[29px] w-[29px] ml-4 flex items-center justify-center decrement-btn absolute left-0 hover:bg-yellow-primary"></button>')
        ...
        .on('click', function (e) {
          ...
          $input.val(Math.max(min || 1, value - 1));
