<?php
/**
 * Our Story slider (shared home + about) — legacy layout + decorative SVGs.
 */
$post_id = (int) get_query_var('matrix_rd_story_post_id');
$stories = get_query_var('matrix_rd_stories');

if (! $post_id || ! is_array($stories) || $stories === []) {
    return;
}

$bg_desktop = matrix_rd_acf_image_url(get_field('background_image', $post_id));
$bg_mobile  = matrix_rd_acf_image_url(get_field('mobile_background_image', $post_id));
$bg_initial = $bg_mobile !== '' ? $bg_mobile : $bg_desktop;
$title_mob  = (string) get_field('title_mob', $post_id);
$span_one_mob = (string) get_field('span_one_mob', $post_id);
$span_two_mob = (string) get_field('span_two_mob', $post_id);
$description_mob = (string) get_field('description_mob', $post_id);
$clip_black = 'clip0_our_story_black_' . $post_id;
$clip_bottom_desk = 'clip0_our_story_bottom_desk_' . $post_id;
$clip_bottom_mob = 'clip0_our_story_bottom_mob_' . $post_id;
$clip_top_desk = 'clip0_our_story_top_desk_' . $post_id;

/**
 * @param array<string, mixed> $story
 */
$render_our_story_card = static function (string $mode, array $story, int $index, bool $is_first): void {
    $img     = matrix_rd_acf_image_url($story['image'] ?? null);
    $img_mob = matrix_rd_acf_image_url($story['image_mobile'] ?? null);
    $donut   = matrix_rd_acf_image_url($story['donut_img'] ?? null);

    if ($mode === 'mobile') {
        ?>
      <li class="splide__slide story-item flex w-[309px] max-w-[309px] shrink-0 flex-col items-center justify-start">
        <?php if ($img_mob) : ?>
          <div class="relative w-[309px]">
            <img src="<?php echo esc_url($img_mob); ?>" alt="<?php echo esc_attr((string) ($story['title'] ?? '')); ?>" class="mobileimg block h-[408px] w-[309px] rounded-[8px] object-cover" />
            <div class="absolute bottom-0 left-0 flex w-full p-4">
              <span class="font-laca text-mob-md-font font-light text-white"><?php echo esc_html((string) ($story['timeline_text'] ?? '')); ?></span>
            </div>
          </div>
        <?php endif; ?>
        <div class="button-dashes mt-4 flex h-[80px] w-[200px] shrink-0 items-center justify-center border-radius-large bg-yellow-primary text-black-full">
          <?php if ($donut) : ?>
            <img class="h-[40px] w-[40px]" src="<?php echo esc_url($donut); ?>" alt="" />
          <?php endif; ?>
          <span class="ml-4 text-center font-reg420 text-md-font"><?php echo esc_html((string) ($story['timeline_button'] ?? '')); ?></span>
        </div>
      </li>
        <?php
        return;
    ...
    ?>
      <li
        class="story-item relative flex h-full w-full items-center justify-center xl:left-12<?php echo $is_first ? ' current' : ''; ?>"
        title="<?php echo esc_attr((string) ($story['title'] ?? '')); ?>"
      >
        <?php if ($img) : ?>
          <img src="<?php echo esc_url($img); ?>" alt="" class="h-full w-full rounded-[10px] object-cover laptop:border-12 laptop:border-white" />
        <?php endif; ?>
      </li>
    <?php
};
...
<section
  id="ourstory"
  class="our-story relative z-50 h-auto bg-black-full bg-contain bg-right max-lg:mb-20 max-lg:mt-10 max-lg:pb-40 max-lg:pt-40 max-md:bg-cover bg-repeat lg:bg-center laptop:h-[900px]"
  <?php if ($bg_initial !== '') : ?>
    style="background-image:url('<?php echo esc_url($bg_initial); ?>');"
  <?php endif; ?>
>
  <svg class="absolute left-0 top-0 z-40 flex lg:hidden" width="100%" viewBox="0 0 390 96" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  <div class="absolute inset-0 h-full w-full bg-black-full opacity-8"></div>

  <div class="relative z-40 mx-auto flex h-auto max-w-max-site flex-col items-start justify-center lg:h-full laptop:flex-row">
    <div class="our-story-cards-desktop relative hidden h-full w-full laptop:order-1 laptop:block laptop:w-3/5 xxl:w-3/5">
      <ul class="cards relative m-auto flex h-full w-full list-none flex-row flex-nowrap items-center justify-between">
        <svg class="yellow_donut pointer-events-none" xmlns="http://www.w3.org/2000/svg" width="123" height="122" viewBox="0 0 123 122" fill="none" aria-hidden="true">
        ...
        <?php foreach ($stories as $index => $story) :
            $render_our_story_card('desktop', $story, $index, $index === 0);
        endforeach; ?>
      </ul>
    </div>

    <div id="our-story-mobile" class="our-story-cards-mobile splide order-1 w-full laptop:hidden" aria-label="<?php esc_attr_e('Our story timeline', 'matrix-starter'); ?>">
      <div class="splide__track">
        <ul class="splide__list">
          <?php foreach ($stories as $index => $story) :
              $render_our_story_card('mobile', $story, $index, $index === 0);
          endforeach; ?>
        </ul>
      </div>
    </div>
    ...
    <div class="text-contents relative order-2 hidden h-full w-full items-center justify-center px-4 laptop:order-3 laptop:flex laptop:-left-10 laptop:w-4/12 laptop:px-0 xxl:-left-24">
      <div class="story-arrows relative z-50 mx-auto hidden h-full w-full max-w-[520px] items-center gap-3 laptop:flex">
        <a href="#" class="arrow prev flex h-[47px] w-[47px] shrink-0 cursor-pointer items-center justify-center rounded-full bg-white text-sm-md-font font-bold hover:bg-yellow-primary" aria-label="<?php esc_attr_e('Previous slide', 'matrix-starter'); ?>">‹</a>

        <div class="our-story-text-track relative min-h-[280px] min-w-0 flex-1">
          <?php foreach ($stories as $index => $story) : ?>
            <div
              class="text-content absolute inset-0 flex flex-col items-start justify-center<?php echo $index === 0 ? ' active' : ''; ?>"
        ...
        <a href="#" class="arrow next flex h-[47px] w-[47px] shrink-0 cursor-pointer items-center justify-center rounded-full bg-white text-sm-md-font font-bold hover:bg-yellow-primary" aria-label="<?php esc_attr_e('Next slide', 'matrix-starter'); ?>">›</a>
      </div>

      <svg class="black_donut pointer-events-none" xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128" fill="none" aria-hidden="true">
        <g clip-path="url(#<?php echo esc_attr($clip_black); ?>)">
        ...
        <defs>
          <clipPath id="<?php echo esc_attr($clip_black); ?>">
  ...
  <div class="absolute -bottom-4 left-0 right-0 z-50 hidden w-full overflow-hidden laptop:block">
    <svg width="100%" height="117" viewBox="0 0 1726 107" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <g clip-path="url(#<?php echo esc_attr($clip_bottom_desk); ?>)">
        <ellipse cx="863" cy="409" rx="1481" ry="405" fill="white" />
      </g>
      <defs>
        <clipPath id="<?php echo esc_attr($clip_bottom_desk); ?>">
  ...
  <svg class="absolute -bottom-0 right-0 hidden w-full lg:flex" viewBox="0 0 1726 107" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g clip-path="url(#<?php echo esc_attr($clip_top_desk); ?>)">
      <ellipse cx="863" cy="-302" rx="1481" ry="405" transform="rotate(-180 863 -302)" fill="white" />
    </g>
    <defs>
      <clipPath id="<?php echo esc_attr($clip_top_desk); ?>">
...
<style>
  @media (min-width: 768px) {
    #ourstory.our-story {
      background-image: url('<?php echo esc_url($bg_desktop); ?>') !important;
    }
  }
</style>
<?php endif; ?>
...
<style>
  ...
  @media (width >= 1250px) {
    ...
    .our-story ul.cards li.story-item {
      position: absolute;
      max-width: 680px;
      max-height: 440px;
      opacity: 0;
    ...
    .our-story ul.cards li.story-item.current {
      z-index: 2;
      opacity: 1;
      transform: translateX(0) rotate(1deg);
    }

    .our-story ul.cards li.story-item.current + li {
      z-index: 1;
      opacity: 1;
    ...
    .our-story ul.cards li.story-item.current + li img {
      border-color: #ffed56;
    }

    .our-story ul.cards li.story-item.current + li ~ li {
      z-index: 0;
      opacity: 1;
    ...
    .our-story ul.cards li.story-item.current + li ~ li img {
      border-color: #ffed56;
    }

    .our-story .yellow_donut {
      position: absolute;
      right: 26%;
      top: 12%;
      z-index: 30;
    ...
    .our-story .text-contents .story-arrows {
      position: relative;
      top: auto;
      width: 100%;
      max-width: 100%;
    ...
    .our-story .our-story-text-track .text-content {
      position: absolute;
      inset: 0;
    }

    .our-story .our-story-text-track .text-content.active {
      position: relative;
  ...
  @media (width <= 1250px) {
    ...
    .our-story-cards-mobile {
      visibility: visible;
    }

    .our-story-cards-mobile .splide__track {
      overflow: visible;
    }

    .our-story-cards-mobile .splide__list {
      align-items: flex-start;
    }

    .our-story-cards-mobile .splide__slide {
      display: flex;
      flex-direction: column;
      align-items: center;
    }

    .our-story-cards-mobile .button-dashes {
      position: relative;
    }

    .our-story-cards-mobile .button-dashes::before,
    .our-story-cards-mobile .button-dashes::after {
      content: '';
      position: absolute;
      top: 50%;
      height: 4px;
