...
<section class="w-full kudos-flexi">
    ...
    <div class="flex justify-center w-full px-4 mx-auto text-center bg-black-full">
        <?php if (is_array($kudosMainImage) && !empty($kudosMainImage['url'])) : ?>
        <img class="w-full object-contain bg-black-full relative -top-10 max-w-[852.196px] h-[92px] mobile:h-[129px] laptop:h-[219px]" src="<?php echo esc_url($kudosMainImage['url']); ?>" alt="<?php echo esc_attr($kudosMainImage['alt'] ?? 'Testimonials Image'); ?>">
        <?php elseif (is_string($kudosMainImage) && !empty($kudosMainImage)) : ?>
        <img class="w-full object-contain bg-black-full relative -top-10 text-center max-w-[852.196px] h-[92px] mobile:h-[129px] laptop:h-[219px]" src="<?php echo esc_url($kudosMainImage); ?>" alt="Testimonials Image">
        <?php endif; ?>
</div>
    <div class="relative z-10 w-full bg-black-full">
        <div class="flex flex-col w-full max-w-[1368px] mx-auto px-4">
            <?php if ($kudosTitle) : ?>
<<?php echo $kudosHeadingTag; ?> class="leading-10 text-white text-md-font lg:text-lg-font font-reg420"><?php echo $kudosTitle; ?></<?php echo $kudosHeadingTag; ?>>
            <?php endif; ?>
<?php if ($kudosText) : ?>
<div class="pb-5 font-light text-white font-laca text-sm-font mobile:text-sm-md-font"><?php echo $kudosText; ?>
            </div>
            <?php endif; ?>
</div>
        <div id="testimonial-slider" class="relative z-50 flex flex-row justify-start w-full mx-auto splide flex-flow flex-nowrap">
            <div class="splide__track w-full max-w-[1530px] ml-auto">
                <div class="splide__list flex justify-start max-w-[1530px] cursor-pointer">
                    <?php if ($selectedKudos) : ?>
<?php foreach ($selectedKudos as $testimonial) : ?>
<div class="splide__slide item p-6 border-3 border-white border-solid rounded-md-32 w-full mobile:w-[497px] mobile:min-w-[497px] h-auto mobile:h-[497px] flex justify-between flex-col">
                        ...
                        <div class="flex flex-col">
                            <span class="text-white text-xs-font font-laca"><?php echo get_the_title($testimonial->ID); ?></span>
                            <?php
$kudosJob = get_field('kudos_job', $testimonial->ID);
                            $kudosImage = get_field('kudos_image', $testimonial->ID);
                            ?>
<span class="pb-6 text-white text-xs-font mobile:text-xs-font font-laca"><?php echo $kudosJob; ?></span>
                            <?php if ($kudosImage) : ?>
<img class="w-[40px] h-[40px]" width="40" height="40" src="<?php echo $kudosImage['url']; ?>" alt="<?php echo $kudosImage['alt'] ?? 'Kudos Image'; ?>">
                            <?php endif; ?>
</div>
                    </div>
                    <?php endforeach; ?>
