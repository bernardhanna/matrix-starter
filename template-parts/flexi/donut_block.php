...
">
        <?php if (have_rows('donuts') : ?>
)
        <?php while (have_rows('donuts') : ?>
) <?php
(the_row())
        <?php
($image = get_sub_field('donut_image'))
        <?php
($link = get_sub_field('donut_link'))
        <div class="flex w-full tablet-sm:w-31-5 border-4 border-black relative overflow-hidden h-[400px] lg:h-[345px] rounded-sm-12 mx-auto justify-center mb-6 lg:mb-0">
            <img src="<?php echo $image; ?>" class="absolute w-full h-full object-cover" alt="Donut Image">
            <a href="<?php echo $link['url']; ?>" class="flex rounded-lg-x flex-row absolute bottom-5 cursor-pointer bg-white hover:bg-yellow-primary w-90 macbook:w-[348.18px] h-[40px] lg:h-[77px] text-sm-font lg:text-1lg-font font-medium text-black-full items-center justify-center lg:justify-start" target="<?php echo $link['target']; ?>">
