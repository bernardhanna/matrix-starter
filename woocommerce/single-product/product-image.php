...
$has_gallery_images = ! empty($image_ids);

$wrapper_classes = apply_filters(
    'woocommerce_single_product_image_gallery_classes',
    array(
        'woocommerce-product-gallery',
        'woocommerce-product-gallery--' . ($has_gallery_images ? 'with-images' : 'without-images'),
        'woocommerce-product-gallery--columns-' . absint($columns),
        'images',
        'rd-product-gallery-wrap',
    )
);
?>
<div class="w-full tablet-sm:w-49 <?php echo esc_attr(implode(' ', array_map('sanitize_html_class', $wrapper_classes))); ?>" data-columns="<?php echo esc_attr($columns); ?>">
    ...
    <div class="rd-product-gallery-stage">
        ...
        <?php endif; ?>
        <?php matrix_rd_render_product_gallery_chrome($product); ?>
    </div>
