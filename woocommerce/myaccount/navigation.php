...
<div class="<?php echo is_user_logged_in() ? 'pb-12' : 'bg-white'; ?>">
    ...
    <div class="mx-auto lg:max-w-max-1549 pt-10">
        <nav class="woocommerce-MyAccount-navigation" aria-label="<?php esc_attr_e('Account pages', 'matrix-starter'); ?>">
            <ul class="w-full flex justify-start lg:justify-center flex-nowrap overflow-x-auto flex-row ">
                <?php foreach (wc_get_account_menu_items() as $endpoint => $label) : ?>
                    <li class="active:text-yellow-primary border-solid border-grey-subdued border-b-2 active:border-yellow-primary px-4 pt-4 <?php echo esc_attr(wc_get_account_menu_item_classes($endpoint)); ?>">
