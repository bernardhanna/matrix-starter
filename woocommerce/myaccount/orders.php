        ...
        ?>
            <div class="flex flex-col mb-5 bg-white small:flex-row rounded-all-sides">
                <div class="hidden w-full small:flex small:flex-row small:w-5/12">
                    <?php
                    // First order item with a featured image, else WooCommerce placeholder.
                    $card_image_url = '';
                    $card_image_alt = $order->get_order_number();
                    foreach ($order->get_items() as $item) {
                        $product = $item->get_product();
                        if ($product && $product->get_image_id()) {
                            $card_image_url = wp_get_attachment_image_url($product->get_image_id(), 'woocommerce_thumbnail');
                            $card_image_alt = $product->get_name();
                            break;
                        }
                    }
                    if (!$card_image_url && function_exists('wc_placeholder_img_src')) {
                        $card_image_url = wc_placeholder_img_src('woocommerce_thumbnail');
                    }
                    if ($card_image_url) {
                        echo '<img class="rounded-left-sides w-full h-auto object-cover" src="' . esc_url($card_image_url) . '" alt="' . esc_attr($card_image_alt) . '">';
                    }
                    ?>
                </div>
        ...
        <table class="w-full rounded-lg woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
            ...
            <tbody>
                ...
                foreach ($customer_orders->orders as $customer_order) {
                    ...
                    echo
                    '<td class="px-10 py-2">';
                    // Insert product thumbnail (first order item with an image, else placeholder).
                    $image_url = '';
                    $image_alt = $order->get_order_number();
                    ...
                    if ($image_url) {
                        echo '<img class="rounded-all-sides h-[44px] w-[55px] object-cover" src="' . esc_url($image_url) . '" alt="' . esc_attr($image_alt) . '">';
                    }
                    echo '</td>';
