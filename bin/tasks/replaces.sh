...
# THE magical sed command (goes through every single file in theme folder and searchs and replaces every air instance with THEME_NAME):
for i in `grep -rl matrix-starter * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;matrix-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl Matrix-starter * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Matrix-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl matrix * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;matrix-starter;${THEME_NAME};" $i $i; done
for i in `grep -rl matrix * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;matrix_starter_;${THEME_NAME}_;" $i $i; done
for i in `grep -rl matrix * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Air_light_;${THEME_NAME}_;" $i $i; done
for i in `grep -rl matrixdev * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;airdev.test;${PROJECT_NAME}.test;" $i $i; done
for i in `grep -rl matrixwptheme * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;airwptheme.com;${PROJECT_NAME}.fi;" $i $i; done
